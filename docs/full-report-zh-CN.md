...
>
> **公开版脱敏说明：** 本 GitHub 版本已对机器唯一或可关联标识进行脱敏。完整 MAC、Serial Number、Product/SKU 等不在公开文档中披露；如后续文档涉及 UUID、Asset Tag、用户名、账户信息、本地路径或其它可关联信息，也将按同一原则处理。固件版本、芯片型号、地址偏移及用于技术验证的哈希值原则上保留，除非它们本身会暴露不应公开的机器专属数据。
